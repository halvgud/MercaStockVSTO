       ...
       public static void Deletecmdserver(string deletePath, string directory)
        ...
        public static List<T> JsonaListaGenerica<T>(IRestResponse json)
        {
            ...
            try
            {
                ...
                var xy = desSerializer.Deserialize<Entidad.ClaseGenerica<T>>(json);
                x = xy.data;
                return x;
            }
            catch (Exception e)
        ...
        public static void BorrarSeleccion(DataGridView ingredientes)
        {
            ...
            var result = ingredientes.DataSource as List<Entidad.Articulo.Basica>;
        ...
        public static void CargarComboBox(Form f1,ComboBox cb,IRestResponse json)
        {
            f1.Invoke(new MethodInvoker(delegate () 
            {
                var bindingSource1 = new BindingSource
                {
                    DataSource = JsonaListaGenerica<Entidad.Generico>(json)
                };
                cb.DataSource = bindingSource1;
                cb.DisplayMember = "Nombre";
                cb.ValueMember = "idSucursal";
                cb.SelectedIndex = 0;
            }));
        }
    }
